...
const ajax = (url, method, data = null, responseType = "text") => {
...
const filterString = (string, type) => {
    var sanitized;
    switch (type) {
        case 'string':
...
const validateString = (string, type) => {
    var validated;
    switch (type) {
        case 'email':
            validated = /^[A-Za-z0-9á-úÁ-Ú]+@[a-z0-9]+.[a-z]+(.[a-z]+)?$/ig.test(string);
            break;
...
const addLeftZeros = (text, quantity) => {
    return text.padStart(quantity, "0");
}

//Quita los ceros a la izquierda - RetaxMaster
const removeLeftZeros = text => {
    return text.replace(/^0+/, "");
}

//Convierte un string a formato URL Amigable - RetaxMaster
const convertStringToUrl = word => {
    var wordCleaned;
    wordCleaned = filterString(word, "remove_special_chars_high");
    wordCleaned = filterString(wordCleaned, "string").split(" ").join("-");
    wordCleaned = removeAccent(wordCleaned);
...
const simplifyObject = (object, index) => {
    const newObject = {};
    let i = 0;
...
const simplifyArray = (array, index) => {
    const newArray = [];
    array.forEach(item => {
...
const restoreUrlValue = (word, arrayToCompare = []) => {
    ...
    if (count(arrayToCompare) > 0) {
        ...
        const posibleWords = filterArrayUrl(arrayToCompare);
